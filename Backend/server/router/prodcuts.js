...
router.put("/:id" , (request , response) =>{
  let id = request.params.id;
  const {product_name, detail, Price } = request.body;
  console.log(request.body)
  const sql = `update products set product_name = ? , detail = ? , Price = ? where product_id = ? `
db.query(sql , [product_name , detail , Price , id] , (error, data) => {
  response.send(utils.createResult(error, data))
})
